...
from my_eval.src.train_module.DeprecateInferenceExpRunner import InferenceExpRunner
...
def sub_main(configs, model):
    ...
    """Preprocess data"""
    tokenized_datasets, tokenizer, verifier = preprocess(configs, raw_datasets, wo_cot=configs.data_args.wo_cot)
    ...
    try:
        ...
        exp_runner.run_cot_utility_to_classmate(verifier=verifier, do_wo_cot=configs.data_args.wo_cot)
        # else:
...
@hydra.main(config_path="configs", config_name="cot_utility_to_classmate", version_base=None)
def main(configs):
    """Load model"""
    ...
    if configs.data_args.dataset_name == "EleutherAI/hendrycks_math":
        ...
        for subset in subsets:
            configs.data_args.dataset_name = "EleutherAI/hendrycks_math"        # Reset to full name to avoid appending multiple subset names
