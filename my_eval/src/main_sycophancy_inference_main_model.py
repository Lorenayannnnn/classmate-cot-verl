...
from my_eval.src.train_module.SycophancyInferenceExpRunner import SycophancyInferenceExpRunner
...
def sub_main(configs, model):
    ...
    """Set up trainer"""
    exp_runner = SycophancyInferenceExpRunner(model=model, tokenizer=tokenizer, data_loader=data_loaders,
...
@hydra.main(config_path="configs", config_name="sycophancy_inference_main_model", version_base=None)
def main(configs):
    """Load model"""
    model = load_model(configs)
