...
from my_eval.src.data_module.DataCollator import DataCollator


def load_data_from_hf(file_or_dataset_name, subset_name, split, cache_dir=None):
    """processes data into huggingface dataset"""
    if file_or_dataset_name.endswith("parquet"):
        raw_datasets = load_dataset(
            "parquet",
            data_files=file_or_dataset_name,
            cache_dir=cache_dir,
        )["train"]
    elif not file_or_dataset_name.endswith(".csv") and not (file_or_dataset_name.endswith(".json") or file_or_dataset_name.endswith(".jsonl")):
        raw_datasets = load_dataset(file_or_dataset_name, subset_name, split=split, cache_dir=cache_dir)
