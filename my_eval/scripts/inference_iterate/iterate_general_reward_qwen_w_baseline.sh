...
#bash my_eval/scripts/inference_iterate/iterate_general_reward_qwen_w_baseline.sh
...
_run_seed_task() {
  ...
  # hf_prefix basename: {task}-{base_model}-{method}  e.g. general_reward-Qwen3-0.6B-OURS_llama
  local hf_basename="${hf_prefix##*/}"
  local task_part="${hf_basename%%-*}"                      # general_reward
  local method_str="${hf_basename##*-}"                     # OURS_llama
  local base_model_str="${hf_basename#${task_part}-}"       # Qwen3-0.6B-OURS_llama
