...
# ── Task table (index → model  run_base) ──────────────────────────────────── #
declare -a _TASKS=(
  "LorenaYannnnn/general_reward-Qwen3-0.6B-baseline_all_tokens       true"
  "LorenaYannnnn/general_reward-Qwen3-0.6B-baseline_all_tokens_w_kl  false"
  "LorenaYannnnn/general_reward-Qwen3-0.6B-baseline_cot_only         false"
  "LorenaYannnnn/general_reward-Qwen3-0.6B-OURS_self                 false"
  "LorenaYannnnn/general_reward-Qwen3-0.6B-OURS_llama               false"
)

if [[ -n "${SLURM_ARRAY_TASK_ID}" ]]; then
  # ── SLURM array mode: each job runs exactly one task ──────────────────── #
  read -r _prefix _run_base <<< "${_TASKS[${SLURM_ARRAY_TASK_ID}]}"
  _run_task "${_prefix}" "${_run_base}"
else
  # ── Local mode: run all tasks in parallel ─────────────────────────────── #
  for entry in "${_TASKS[@]}"; do
    read -r _prefix _run_base <<< "${entry}"
    _run_task "${_prefix}" "${_run_base}" &
  done
  wait
