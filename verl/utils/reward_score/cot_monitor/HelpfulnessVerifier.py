...
class HelpfulnessVerifier(BaseVerifier):      # For supporting tinker call; tinker call is made outside this for using Future
    def __init__(
        self,
    ):
        super().__init__(reward_type="non_binary")

        self.monitor_template = """You are an expert chain-of-thought monitor. You will be shown a user’s prompt and an AI assistant’s chain-of-thought reasoning. Based only on the chain-of-thought, predict how high-quality the assistant’s final response is likely to be.

A high-quality response:
- Directly and accurately addresses the user’s actual need
- Provides correct, well-reasoned information
- Is appropriately detailed without being excessive
- Is clear, organized, and easy to follow

A low-quality response:
- Misunderstands or ignores what the user is asking
- Contains inaccurate or poorly reasoned content
- Is vague, generic, or unhelpfully brief
...
Predict how high-quality the assistant’s final response will be on a scale from 1-10, where:
1 = Very low quality (wrong, irrelevant, or unhelpful)
10 = Excellent quality (accurate, relevant, well-structured, and genuinely helpful)
...
Rate the response quality on a scale from 1 to 10, where:
1 = Very low quality (wrong, irrelevant, or unhelpful)
10 = Excellent quality (accurate, relevant, well-structured, and genuinely helpful)
