...
class BaseVerifier(ABC):
    ...
    def _extract_response_text(response, config):
        ...
        parsed, _ = config["renderer"].parse_response(tokens)
        return _renderers.get_text_content(parsed)
...
def get_verifier(data_source=None, **kwargs) -> BaseVerifier:
    ...
    if any("anthropic_sycophancy" in source for source in data_sources):
        ...
        from verl.utils.reward_score.ConfidenceVerifier import ConfidenceVerifier
    ...
    elif any("general_prompt_llm_judge" in source for source in data_sources):
    ...
    # elif any("sycophancy_only" in source for source in data_sources):
