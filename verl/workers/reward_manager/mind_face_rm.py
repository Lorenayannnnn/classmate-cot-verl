...
# ── [MIND-FACE] ───────────────────────────────────────────────────────────────
# Copied from naive.py and registered as "mind_face".
#
# In the mind-face baseline the face model generates only the answer (no CoT),
# because the mind model's CoT is prepended to the face model's prompt.
# Therefore enable_thinking should typically be False: the face's full response
# IS the answer, and no think-tag extraction is needed.
#
# If the face model unexpectedly emits a <think>...</think> block, setting
# enable_thinking=True will still extract the output correctly.
# ── [END MIND-FACE] ──────────────────────────────────────────────────────────
...
# ── [MIND-FACE: CHANGED] registered as "mind_face" instead of "naive" ────────
@register("mind_face")
class MindFaceRewardManager(AbstractRewardManager):
    """Reward manager for the mind-face baseline.

    Scores only the face model's output tokens.  The face model receives
    [prompt + mind_CoT] as its prompt and generates the final answer; this
    manager scores that answer using the LLM judge backend.
    """
    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        """Score the face model's response and return a token-level reward tensor."""
        ...
        # ── Loop 1: decode face responses, collect judge prompts ──────────────
        item_contexts = []
        judge_prompts = []
        ...
        for i in range(len(data)):
            ...
            # ── [MIND-FACE] ────────────────────────────────────────────────────
            # The face model's prompt already contains the mind's CoT, so its
            # "response" is just the final answer.  enable_thinking is normally
            # False here; if True, we still extract the output after </think>.
            if self.enable_thinking:
        ...
        for ctx, judge_output in zip(item_contexts, judge_outputs):
            ...
            if isinstance(score, dict):
                ...
                for key, value in score.items():
                    if key == "score":
                        reward_extra_info["face_model_reward"].append(value)
                    else:
                        reward_extra_info[f"face_model_{key}"].append(value)
            else:
                reward = score

            # ── [MIND-FACE] reward placed on the last valid face-output token ──
        ...
        for ctx in item_contexts:
            if already_print_count < self.num_examine:
                ...
                print("🧠[mind-face raw_prompt]", ctx["raw_prompt"])
                # ── [MIND-FACE] prompt_str now includes the mind's CoT prefix ─
                print("🧠[face prompt (includes mind CoT)]", ctx["prompt_str"])
                print("🧠[face response (answer only)]", ctx["response_str"])
                if self.enable_thinking:
                    print("🧠[face cot (unexpected)]", ctx["main_cot"])
                    print("🧠[face output]", ctx["main_output"])
                if len(ctx["other_debug_dict"]) > 0:
                    print("🧠[other_debug_dict]", ctx["other_debug_dict"])
                print("🧠[face llm judge score]", ctx["reward"])
                print("🧠[face judge_explanation]", ctx["judge_explanation"])
