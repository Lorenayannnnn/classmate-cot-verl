...
@hydra.main(config_path="config", config_name="olmo3_classmate_cot_ppo_trainer", version_base=None)
def main(config):
...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    if task_runner_class is None:
        task_runner_class = ray.remote(num_cpus=1)(ClassmateCoTTaskRunner)  # please make sure main_task is not scheduled on head
...
class ClassmateCoTTaskRunner:
    ...
    def add_classmate_rollout_worker(self, config):
        ...
        # Add the ClassmateWorker class to role mapping
        # Note: Individual workers will be created by the trainer using create_classmate_workers()
    ...
    def run(self, config):
        ...
        # Add classmate workers if classmate CoT is enabled
