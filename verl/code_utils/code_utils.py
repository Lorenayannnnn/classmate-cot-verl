...
def get_successful_tests_fast(
    ...
    # Run tests in parallel with at most 8 workers
    ...
    # Process tests in batches of up to 8
    max_workers = 2
    for batch_start in range(0, len(tests), max_workers):
        batch_end = min(batch_start + max_workers, len(tests))
        processes = []
...
def run_individual_stdio_test_helper(
    program: str, test: dict[str, Any], result_array, index: int, runtimes_array, max_execution_time: float
):
    """Run a single stdio test and store result in shared array at given index"""
    reliability_guard()
    try:
        test_input = test["input"]
        test_output = test["output"]
        timeout = math.ceil(max_execution_time)
        results, runtimes = grade_stdio(program, [test_input], [test_output], timeout)

        if results is not None and len(results) > 0:
            result_array[index] = 1 if results[0] is True else 0
            runtimes_array[index] = runtimes[0] if len(runtimes) > 0 else -1.0
        else:
            result_array[index] = 0
            runtimes_array[index] = -1.0
    except Exception:
...
def get_successful_tests_stdio(
    ...
    # Process tests in batches of up to 8
    max_workers = 2
    for batch_start in range(0, test_ct, max_workers):
        batch_end = min(batch_start + max_workers, test_ct)
        processes = []
        
        for idx in range(batch_start, batch_end):
            test = tests[idx]
            p = multiprocessing.Process(
                target=run_individual_stdio_test_helper,
                args=(program, test, stdio_test_results, idx, stdio_runtimes, max_execution_time),
            )
            p.start()
            processes.append((p, idx))
        
        # Wait for all processes in this batch to complete
        total_timeout = max_execution_time + 5.0
        for p, idx in processes:
