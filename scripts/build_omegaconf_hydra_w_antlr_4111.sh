...
# download the antlr 4.11.1 binary:
...
# update all references to 4.9.3 / 4.9.* to 4.11.1 in omegaconf + hydra only
(
  cd omegaconf
  grep -ErlI '4\.9\.[\*3]' . | xargs sed -E -i 's/4\.9\.[\*3]/4.11.1/g' || true
)
(
  cd hydra
  grep -ErlI '4\.9\.[\*3]' . | xargs sed -E -i 's/4\.9\.[\*3]/4.11.1/g' || true
...
# verify that the Python runtime installed is 4.11.1
